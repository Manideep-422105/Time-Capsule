...
export default function CapsuleCard({
  ...
  return (
    <div
      className={`glass-panel p-6 rounded-xl flex flex-col gap-4 h-auto min-h-[14rem] transition-all hover:-translate-y-1 duration-300 ${
        isUnlocked
    ...
    >
      <div className="flex-1 space-y-3">
        ...
        <div className="flex justify-between items-start">
          <h3 className="font-bold text-lg truncate pr-2 text-white">
            {capsule.title}
          </h3>
          {isUnlocked ? (
        ...
        <p className="text-xs text-gray-400">
          {isReceived
            ? `From: ${capsule.senderName || "Unknown"}`
            : `To: ${capsule.recipientEmail}`}
        </p>

        {/* Message */}
        {isUnlocked && capsule.message && (
          <p className="text-sm text-gray-300 italic">"{capsule.message}"</p>
        )}

...
import { 
  Inbox, Send, User, Search, Plus, 
  LayoutGrid, Lock, Unlock, LogOut, Menu, X 
} from "lucide-react";
...
export default function DashboardClient({ user, sent, received }: DashboardProps) {
  ...
  const unlockedCount = [...sent, ...received].filter(c => new Date() >= new Date(c.unlockDate)).length;
  ...
  return (
    <div className="flex min-h-screen bg-[#050510] text-gray-100 overflow-hidden font-sans">
      ...
      {/* Added logic to show/hide on mobile based on state */}
      <aside className={`
      ...
      `}>
        <div className="p-8 flex justify-between items-center">
            <div>
                <h1 className="text-3xl font-extrabold bg-clip-text text-transparent bg-linear-to-r from-blue-400 to-purple-600 tracking-tighter">
                    TimeVault
                </h1>
                <p className="text-xs text-gray-500 mt-1">Chrono-Storage Protocol</p>
            </div>
            {/* Close button for mobile menu */}
            <button onClick={() => setIsMobileMenuOpen(false)} aria-label="Close Button" className="md:hidden text-gray-400">
                <X className="w-6 h-6" />
            </button>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-2">
            <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === "INBOX" ? "bg-blue-600 text-white shadow-lg shadow-blue-900/40" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
            >
                <Inbox className="w-5 h-5" /> Received Memories
                {received.length > 0 && <span className="ml-auto bg-white/20 px-2 py-0.5 rounded text-xs">{received.length}</span>}
            </button>
            <button 
                onClick={() => { setActiveTab("SENT"); setIsMobileMenuOpen(false); }} 
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === "SENT" ? "bg-purple-600 text-white shadow-lg shadow-purple-900/40" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
            >
                <Send className="w-5 h-5" /> Sent Capsules
                {sent.length > 0 && <span className="ml-auto bg-white/20 px-2 py-0.5 rounded text-xs">{sent.length}</span>}
            </button>
        </nav>

        <div className="p-4 border-t border-white/10">
            <button onClick={() => setShowProfile(true)} className="flex items-center gap-3 w-full p-3 rounded-xl hover:bg-white/5 transition-all group">
                {/* Profile Image Logic */}
                {user.image ? (
                    <img src={user.image} alt={user.name || "User"} className="w-10 h-10 rounded-full border-2 border-blue-500/50" />
                ) : (
      ...
      <main className="flex-1 relative flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-10 glass-panel border-b border-white/5 p-6 flex items-center justify-between gap-4">
            ...
            <Link href="/create" className="hidden md:flex items-center gap-2 bg-white text-black px-5 py-3 rounded-xl font-bold hover:bg-gray-200 transition shadow-[0_0_20px_rgba(255,255,255,0.2)]">
        ...
        <div className="flex-1 p-6 lg:p-10 space-y-8 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard icon={<LayoutGrid />} label="Total Memories" value={totalCapsules} color="text-blue-400" />
                <StatCard icon={<Lock />} label="Locked" value={lockedCount} color="text-gray-400" />
                <StatCard icon={<Unlock />} label="Unlocked" value={unlockedCount} color="text-green-400" />
                
                {/* Dynamic Next Unlock Card */}
                <div className="glass-panel p-5 rounded-2xl border border-white/10 relative overflow-hidden group">
                    ...
                    {nextUnlock ? (
                        <div className="flex flex-col">
                            <span className="text-xl font-bold text-white truncate">{nextUnlock.title}</span>
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold flex items-center gap-2">
                        {activeTab === "INBOX" ? <Inbox className="text-blue-400"/> : <Send className="text-purple-400"/>}
                        ...
                        {search ? (
                            ...
                            <>
                                <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mb-4">
                                    {activeTab === "INBOX" ? <Inbox className="opacity-50"/> : <Send className="opacity-50"/>}
                                </div>
        ...
        <Link href="/create" className="md:hidden fixed bottom-6 right-6 w-14 h-14 bg-blue-600 rounded-full flex items-center justify-center shadow-2xl z-50 text-white">
            ...
            <div className="relative w-full max-w-md bg-[#0a0a15] h-full border-l border-white/10 shadow-2xl p-8 flex flex-col animate-in slide-in-from-right duration-300">
                ...
                <div className="flex justify-between items-center mb-8">
                    ...
                    <button 
                        ...
                        aria-label="Close Profile" // FIXED: Added Aria Label
                ...
                <div className="flex flex-col items-center text-center mb-10">
                    ...
                    {user.image ? (
                        <img src={user.image} alt={user.name || "User"} className="w-24 h-24 rounded-full mb-4 border-4 border-blue-500/30 object-cover" />
                    ) : (
                ...
                <div className="space-y-6 flex-1">
                    <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                        <h4 className="text-sm text-gray-400 uppercase tracking-wider mb-4">Account Stats</h4>
                        <div className="space-y-3">
                            <div className="flex justify-between">
                                <span>Sent</span>
                                <span className="font-mono text-purple-400">{sent.length}</span>
                            </div>
                            <div className="flex justify-between">
                                <span>Received</span>
                                <span className="font-mono text-blue-400">{received.length}</span>
                            </div>
                        </div>
                    </div>
                </div>
